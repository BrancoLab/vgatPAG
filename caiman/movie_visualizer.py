...
def compare_videos(*args, rgb=False, **kwargs):
    """
        Look at a number of videos side by side. 
        Depending on the number of videos and their size it might take a while to 
        load up. 

        Pass a list of paths to memmapped or tiff files
    """

    images = [load_mmapped(fp) if fp.endswith(".mmap") else napari.utils.io.magic_imread(fp) 
                        for fp in args]
    ...
    with napari.gui_qt():
        ...
        for fp, imgs in zip(args, images):
            v.add_image(imgs, name=get_file_name(fp))
